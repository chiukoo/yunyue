...
function delfun( act ){
  ...
  document.d.action = 'deldemo.php?act=sys&id=' + setid ;
    document.d.submit();
 ...
 document.d.action = 'ad_usupdate.php?id=' + setid ;
  ...
  function del(key_num,msg){
	...
	if(confirm("您确定要刪除嗎？")){
	   $.ajax({ 
    url: 'deldemo.php',
	data: {keyNum:key_num,tables:"sys"},
    error: function(xhr) {
...
<div id="container">
  ...
  <div class="breadcrumb">
        <a href="index.php">首頁(Home)</a> :: <a href="sys_list.php">demo設定</a>
      </div>
    <div class="box">
    <div class="heading">
      <h1><img src="image/category.png" alt="" />主頁</h1>
      <div class="buttons"><a href="sys_list_a.php" class="button">新增(Add)</a><a href="javascript:delfun('del');" class="button">刪除(Del)</a></div>	  
	  </div>
    </div>
    <div class="content">
		...
		<?php	for($x=0;$x<$total_num;$x++){	?>
           ...
           <td width="200" align="center"><?php if($title[$x]!=''){echo $title[$x];}?>
            ...
            <td width="200" class="book-img"><a href="<?php echo $url;?>"><?php if($img[$x] !=""){echo '<img width="100" height="50" id="img" src="'.$uploaddir.$img[$x].'">';}?></a>
			  ...
			  <td align="right"><div class="buttons"><a href="sys_list_e.php?num=<?php echo $id[$x];?>" class="button">編輯(Edit)</a>  <a onClick="del(<?php echo $id[$x];?>);" class="button">刪除(Del)</a></div></td>
			</tr>
		<?php	}	?>			  
            </table>
        </div>
