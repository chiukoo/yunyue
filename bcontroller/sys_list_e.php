<?php
 //包含需求檔案 ------------------------------------------------------------------------
	...
	if(isset($_POST['send_data'])&&$_POST['send_data']=='has_post_value' ){
		...
		$uploadfile = $uploaddir.$new_name;
			...
			if (move_uploaded_file($_FILES['img']['tmp_name'], $uploadfile)) {
				$up_dsc ="update `sys_index` set `img`='".$new_name."'  where `id`='".$_POST['id']."'";
	...
	$res_edit_history=$ODb->query($insert_edit_history) or die("新增資料出錯，請聯繫管理員。");
			...
			$up_dsc ="update `sys_index` set `order`='".$_POST['order']."',`title`='".$_POST['title']."',`content`='".$_POST['content1']."',`url`='".$_POST['url']."',`edit_dt`='".$updateDsc."' where `id`='".$_POST['id']."'";
				$res=$ODb->query($up_dsc) or die("更新資料出錯，請聯繫管理員。");
		//歷史紀錄	
...
$sql_dsc = "select * from `sys_index` where `id`='".$_GET['num']."' ";
...
<div id="container">
  ...
  <div class="breadcrumb">
      <a href="index.php">首頁(Home)</a> :: <a href="sys_list.php">主頁</a> :: <a href="#">修改主頁</a>
      </div>
    <div class="box">
    <div class="heading">
      <h1><img src="image/category.png" alt="" /> 修改主頁資訊</h1>
      <div class="buttons"><a onClick="ck_value()" class="button">保存(Save)</a><a href="sys_list.php" class="button">取消(Cancel)</a></div>
    </div>
    <div class="content">
