...
function del(key_num){
 $.ajax({
    url: 'delfunction.php',
	data: {keyNum:key_num,tables:"user"},
    error: function(xhr) {
      ...
      <form action="member.php" method="post" enctype="multipart/form-data" id="form">
		...
		<?php	for($x=0;$x<$total_num;$x++){	?>
            ...
            <td><?php if($sys[$x] !=''){echo $sys[$x];}?></td>
				...
				<td align="right"><div class="buttons"><a href="user_e.php?id=<?php echo $sys_id[$x];?>" class="button">編輯(Edit)</a>  <a onClick="del(<?php echo $sys_id[$x];?>);" class="button">刪除(Del)</a></div></td>
			</tr>
		<?php	}	?>			  
            </table>
        </div>
